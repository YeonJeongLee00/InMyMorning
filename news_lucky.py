...
def info(num=None):
    if request.method == 'GET':
        ...
        category = request.form.getlist('chk_info')
        birth = birth.replace('-', "")
        ...
        prefs = {'profile.default_content_setting_values': {'cookies': 2, 'images': 2, 'plugins': 2, 'popups': 2, 'geolocation': 2, 'notifications': 2, 'auto_select_certificate': 2, 'fullscreen': 2, 'mouselock': 2, 'mixed_script': 2, 'media_stream': 2, 'media_stream_mic': 2,
                                                            'media_stream_camera': 2, 'protocol_handlers': 2, 'ppapi_broker': 2, 'automatic_downloads': 2, 'midi_sysex': 2, 'push_messaging': 2, 'ssl_cert_decisions': 2, 'metro_switch_to_desktop': 2, 'protected_media_identifier': 2, 'app_banner': 2, 'site_engagement': 2, 'durable_storage': 2}}
        options.add_experimental_option('prefs', prefs)
        dv = webdriver.Chrome(
            '/Users/yeon/Downloads/chromedriver', chrome_options=options)
        ...
        for cate in category:
            ...
            for i in result:
                ...
                news_link = i.select(".cluster_item .cluster_text a")[
                    0].attrs['href']
                # print('내용')
                # print(content)
